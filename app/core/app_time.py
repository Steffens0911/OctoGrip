...
def combine_local_date_start_utc(d: date) -> datetime:
    ...
    return datetime.combine(d, time.min, tzinfo=get_app_tz()).astimezone(UTC)


def combine_local_date_end_utc(d: date) -> datetime:
    """Fim do dia `d` no fuso do app (23:59:59.999999), como datetime com tz UTC."""
    return datetime.combine(d, time.max.replace(microsecond=999999), tzinfo=get_app_tz()).astimezone(UTC)
...
def combine_local_date_exclusive_end_utc(d: date) -> datetime:
    ...
    return datetime.combine(d + timedelta(days=1), time.min, tzinfo=get_app_tz()).astimezone(UTC)
