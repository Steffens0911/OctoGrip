  ...
  <div class="layout">
    ...
    <main class="main">
      ...
      <div id="panel-techniques" class="panel">
        ...
        <table><thead><tr><th>Nome</th><th>Slug</th><th>De → Para</th><th>Ações</th></tr></thead><tbody id="tbl-techniques"></tbody></table>
      </div>
      <div id="panel-positions" class="panel">
        ...
        <table><thead><tr><th>Nome</th><th>Slug</th><th>Descrição</th><th>Ações</th></tr></thead><tbody id="tbl-positions"></tbody></table>
      </div>
...
(function() {
  ...
  function showLessonForm(editId) {
    ...
    sel('#f-lesson input[name="id"]').value = editId || ''; sel('#f-lesson input[name="title"]').value = ''; sel('#f-lesson input[name="slug"]').value = ''; sel('#f-lesson input[name="video_url"]').value = ''; sel('#f-lesson textarea[name="content"]').value = ''; sel('#f-lesson input[name="order_index"]').value = '0';
    if (editId) get('/lessons/' + editId).then(l => { sel('#f-lesson select[name="technique_id"]').value = l.technique_id; sel('#f-lesson input[name="title"]').value = l.title; sel('#f-lesson input[name="slug"]').value = l.slug; sel('#f-lesson input[name="video_url"]').value = l.video_url || ''; sel('#f-lesson textarea[name="content"]').value = l.content || ''; sel('#f-lesson input[name="order_index"]').value = l.order_index; }).catch(() => {});
  }
  document.querySelector('[data-action="new-lesson"]').addEventListener('click', () => showLessonForm(null));
  document.getElementById('f-lesson').addEventListener('submit', async e => { e.preventDefault(); const fd = new FormData(e.target); const id = fd.get('id'); hideMsg('msg-lesson'); const body = { technique_id: fd.get('technique_id'), title: fd.get('title'), slug: fd.get('slug'), video_url: fd.get('video_url') || null, content: fd.get('content') || null, order_index: parseInt(fd.get('order_index'), 10) }; try { if (id) { await put('/lessons/' + id, body); showMsg('msg-lesson', 'Lição atualizada.', true); } else { await post('/lessons', body); showMsg('msg-lesson', 'Lição criada.', true); $('form-lesson').classList.add('hidden'); loadLessons(); } } catch (err) { showMsg('msg-lesson', (Array.isArray(err.detail) ? err.detail.map(x => x.msg).join(' ') : err.detail) || err.message || 'Erro', false); } });
  ...
  async function loadTechniques() {
    ...
    tbody.innerHTML = list.map(t => { const fromP = positions.find(x => x.id === t.from_position_id); const toP = positions.find(x => x.id === t.to_position_id); return '<tr><td>' + escapeHtml(t.name) + '</td><td>' + escapeHtml(t.slug) + '</td><td>' + (fromP ? fromP.name : '') + ' → ' + (toP ? toP.name : '') + '</td><td class="actions"><button class="btn btn-secondary btn-sm" data-edit-technique="' + t.id + '">Editar</button><button class="btn btn-danger btn-sm" data-delete-technique="' + t.id + '">Excluir</button></td></tr>'; }).join('');
  ...
  function showTechniqueForm(editId) {
    ...
    sel('#f-technique input[name="id"]').value = editId || ''; sel('#f-technique input[name="name"]').value = ''; sel('#f-technique input[name="slug"]').value = ''; sel('#f-technique textarea[name="description"]').value = '';
    if (editId) get('/techniques/' + editId).then(t => { sel('#f-technique input[name="name"]').value = t.name; sel('#f-technique input[name="slug"]').value = t.slug; sel('#f-technique textarea[name="description"]').value = t.description || ''; sel('#f-technique select[name="from_position_id"]').value = t.from_position_id; sel('#f-technique select[name="to_position_id"]').value = t.to_position_id; }).catch(() => {});
  }
  document.querySelector('[data-action="new-technique"]').addEventListener('click', () => showTechniqueForm(null));
  document.getElementById('f-technique').addEventListener('submit', async e => { e.preventDefault(); const fd = new FormData(e.target); const id = fd.get('id'); hideMsg('msg-technique'); const body = { name: fd.get('name'), slug: fd.get('slug'), description: fd.get('description') || null, from_position_id: fd.get('from_position_id'), to_position_id: fd.get('to_position_id') }; try { if (id) { await put('/techniques/' + id, body); showMsg('msg-technique', 'Técnica atualizada.', true); } else { await post('/techniques', body); showMsg('msg-technique', 'Técnica criada.', true); $('form-technique').classList.add('hidden'); loadTechniques(); } } catch (err) { showMsg('msg-technique', (Array.isArray(err.detail) ? err.detail.map(x => x.msg).join(' ') : err.detail) || err.message || 'Erro', false); } });
  ...
  async function loadPositions() {
    ...
    tbody.innerHTML = list.map(p => '<tr><td>' + escapeHtml(p.name) + '</td><td>' + escapeHtml(p.slug) + '</td><td>' + escapeHtml((p.description || '').slice(0, 60)) + (p.description && p.description.length > 60 ? '…' : '') + '</td><td class="actions"><button class="btn btn-secondary btn-sm" data-edit-position="' + p.id + '">Editar</button><button class="btn btn-danger btn-sm" data-delete-position="' + p.id + '">Excluir</button></td></tr>').join('');
  ...
  function showPositionForm(editId) {
    ...
    sel('#f-position input[name="id"]').value = editId || ''; sel('#f-position input[name="name"]').value = ''; sel('#f-position input[name="slug"]').value = ''; sel('#f-position textarea[name="description"]').value = '';
    if (editId) get('/positions/' + editId).then(p => { sel('#f-position input[name="name"]').value = p.name; sel('#f-position input[name="slug"]').value = p.slug; sel('#f-position textarea[name="description"]').value = p.description || ''; }).catch(() => {});
  }
  document.querySelector('[data-action="new-position"]').addEventListener('click', () => showPositionForm(null));
  document.getElementById('f-position').addEventListener('submit', async e => { e.preventDefault(); const fd = new FormData(e.target); const id = fd.get('id'); hideMsg('msg-position'); const body = { name: fd.get('name'), slug: fd.get('slug'), description: fd.get('description') || null }; try { if (id) { await put('/positions/' + id, body); showMsg('msg-position', 'Posição atualizada.', true); } else { await post('/positions', body); showMsg('msg-position', 'Posição criada.', true); $('form-position').classList.add('hidden'); loadPositions(); } } catch (err) { showMsg('msg-position', (Array.isArray(err.detail) ? err.detail.map(x => x.msg).join(' ') : err.detail) || err.message || 'Erro', false); } });
