...
from app.services.professor_service import create_professor, delete_professor, get_professor, get_professor_by_email, list_professors, update_professor

router = APIRouter()


@router.get("", response_model=list[ProfessorRead])
async def professors_list(academy_id: UUID | None = Query(None), db: AsyncSession = Depends(get_db)):
...
async def professor_create(body: ProfessorCreate, db: AsyncSession = Depends(get_db)):
    ...
    return await create_professor(db, name=body.name, email=body.email, academy_id=body.academy_id)


@router.patch("/{professor_id}", response_model=ProfessorRead)
async def professor_update(professor_id: UUID, body: ProfessorUpdate, db: AsyncSession = Depends(get_db)):
    payload = body.model_dump(exclude_unset=True)
    updated = await update_professor(db, professor_id, name=payload.get("name"), email=payload.get("email"), academy_id=payload.get("academy_id"))
    if not updated:
        raise HTTPException(status_code=404, detail="Professor não encontrado.")
    return updated
