...
- **`ApiService.getMeHeaderStats`** (`GET /me/header_stats`): snapshot agregado (nível, XP no nível, threshold, logo/horários e flags da academia) com cache de cliente (**45s**), usado para renderizar o header sem depender de múltiplas chamadas. Missão da semana (`GET /mission_today/week`) partilha TTL curto alinhado (**45s**): o *pull-to-refresh* pode servir resposta em memória se ainda estiver válida (sem invalidar à toa).
- **Invalidação de cache do header**: após ações que alteram pontuação/estado do header (`postMissionComplete`, `postLessonComplete`, `completeTrainingVideo`, `patchMeGalleryVisible`, upload/patch da academia), o cliente limpa `GET:$baseUrl/me/header_stats`.
...
- **`StudentHomeScreen`**: na primeira carga (e refresh completo), **`GET /me/header_stats`**, **`GET /mission_today/week`**, meta coletiva e confirmações pendentes disparam em **paralelo** (`Future.wait`); só se o header falhar é que corre o fallback (`getUserPoints` + `getAcademy`). O **vídeo do dia** carrega **depois** dessa onda (não bloqueia nível/brasão/missões). O refresh inicial não limpa o snapshot visual do header; fallback para rotas antigas apenas se o agregado falhar.
- **Snapshot em disco (`StudentHomeSnapshotStore`)**: após um carregamento bem-sucedido, o cliente grava em `SharedPreferences` o JSON do header agregado + semana de missões (chave por `user_id`, com validação de `academy_id`/`level` e expiração de **7 dias**). No próximo arranque (ex.: reload no browser), a home **hidrata** esse snapshot antes da rede e põe `_loading` a falso logo que há dados, reduzindo ecrã vazio. Em **logout**, `AuthService.logout` chama `StudentHomeSnapshotStore.clearAll()`.
- **Recarregar sem “piscar”**: ao repetir a aba **Campo de treinamento** (`refreshTrigger` no `MainShell`) e no **pull-to-refresh**, a home usa `_load(silent: true)` — não força `_loading` nem apaga a celebração de missão; continua a refazer pedidos à API (que podem devolver cache TTL se ainda válidos).
- **Pré-carregamento da logo**: quando `logo_url` muda via header stats, o app chama `precacheImage` em **segundo plano** (`unawaited`) para não atrasar a conclusão dos pedidos HTTP; o brasão já usa a URL na mesma frame.
...
| `getMeHeaderStats`             | GET /me/header_stats         |
